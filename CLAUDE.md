...
- **Tailwind v4** דרך `@tailwindcss/vite` (פלאגין Vite ב-`astro.config.mjs`; ה-integration `@astrojs/tailwind` הוסר כי אינו תומך ב-Astro 6). טוקני עיצוב ב-`@theme` בתוך `src/styles/global.css`; glass/neon/reveal שם גם כן.
...
- **אבטחה**: `src/middleware.ts` שומר על `/admin/*`, מוסיף cache לעמודים ציבוריים (ו-`private, no-store` ל-admin/api), ומחיל כותרות אבטחה על כל תגובה — CSP מחמיר (`script-src 'self'` בפרודקשן), anti-clickjacking ו-hardening — מ-`src/lib/security.ts` (`securityHeaders(isDev)`; בדיקות ב-`tests/security.test.ts`). **בהוספת סקריפט inline או משאב חיצוני (CDN/דומיין/fetch) — עדכן את ה-CSP ב-`src/lib/security.ts`, אחרת הדפדפן יחסום אותו.** הגארד של `/admin` רץ על נתיב מנורמל (`normalizeGuardPath`, `src/lib/pathGuard.ts`) כדי למנוע עקיפה בקידוד-URL; תוכן CMS המוצג ב-`set:html` (כותרות הירו/CTA/PageHeader) **חייב** לעבור `sanitizeInline` (`src/lib/sanitize.ts`). RLS ב-Supabase (קריאה ציבורית, כתיבה למנהלים בלבד); `src/lib/admin.ts` (`isAdmin`).

## Design tokens (sci-fi / gold-neon) — שמור על אחידות
- צבעים (`@theme` ב-`global.css`): `space #0b0a07` (רקע), `space2 #100d09`, `fg #f1ebdf` (טקסט), `mute #a59b86`, `cyan #ffd24a` (אקסנט זהב-נאון), `magenta #ff9d2f` (ענבר), `violet #d4a017`.
